...
class VirtualSD:
    def __init__(self, config):
        ...
        # Optional responsive temperature wait support.  The current G-Code
        # call stack remains blocked, while selected heater/cancel commands
        # may run through a controlled G-Code dispatch channel.
    ...
    def begin_temperature_wait(self, sensor, check_ready, get_target):
        if (not self.cancelable_temperature_wait
            or not self.gcode.get_mutex().is_owned_by_current()):
            return False
        if self.temperature_wait is not None:
            raise self.gcode.error(
                "A temperature wait is already active")
        self.temperature_wait = {
            'sensor': sensor,
            'check_ready': check_ready,
            'get_target': get_target,
        }
        logging.info("Responsive temperature wait started for %s", sensor)
        try:
            self.gcode.wait_for_temperature(
                check_ready, self.temperature_wait_check_interval)
        finally:
            self._clear_temperature_wait()
        return True
    def _clear_temperature_wait(self):
        if self.temperature_wait is not None:
            logging.info("Responsive temperature wait finished for %s",
    ...
    def work_handler(self, eventtime):
        ...
        while not self.must_pause_work:
            ...
            try:
                result = self.gcode.run_script(line)
                if result is self.gcode.SCRIPT_CANCELLED:
